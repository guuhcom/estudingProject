...
namespace projetoIntegrador.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> GoHome()
        {
        ...
        public async Task<IActionResult> Logar([Bind] AccountViewModel lvm)
        {
            ...
            try
            {
                ...
                Usuario user = CreateUser(lvm.Email, lvm.Password);
                    ...
                    if (idCarteiraSelecionada >= 0 && idCarteiraSelecionada <= carteiras.Count)
                    {
                        ...
                        if (IsValidPassword(user, lvm.Password))
                        {
                            ...
                            if (principal.Identity.IsAuthenticated)
                            {
                                ...
                                //        break;
                                //    case 1:
                                //        return RedirectToAction("Index", "Home", new { area = "AgroxUser" });
                                //        break;
                                //    default:
        ...
        private Usuario CreateUser(string login, string password) => new Usuario() { LOGIN_USU = login, SENHA_USU = password.Trim() };

        private Usuario GetUsuario(string login) => (Usuario)_usuario.GetUsuarioBayLogin (login.Trim());
        ...
        private bool IsValidPassword(Usuario user, string password)
        {
            ...
            return (password == user.SENHA_USU.ToString()) ? true : false;
        }
